...
public class UserManagementService implements IUserManagementService {
    ...
    @Transactional
    @Override
    public UserInfo updateFcmToken(HttpServletRequest request, UpdateFcmTokenRequest updateRequest) {
        if (!StringUtils.hasText(updateRequest.getFcmToken())) {
        ...
        user.updateFcmToken(updateRequest.getFcmToken().trim());
        User savedUser = userRepository.save(user);
    ...
    @Transactional
    @Override
    public UserInfo updateNotificationPreferences(HttpServletRequest request,
                                                  NotificationPreferencesRequest preferencesRequest) {
        Long userId = jwtHelperService.getCurrentUserIdFromRequest(request);
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new CustomException("User not found"));

        if (preferencesRequest.getPushNotificationsEnabled() != null) {
            user.setPushNotificationsEnabled(preferencesRequest.getPushNotificationsEnabled());
        }
    ...
    @Transactional
    @Override
    public void clearFcmToken(HttpServletRequest request) {
        Long userId = jwtHelperService.getCurrentUserIdFromRequest(request);
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new CustomException("User not found"));
